...
// 將檔案內所有 ?v=202607281155 統一替換為新版本號
...
function executeFinalSave() {
    ...
    setTimeout(() => {
        ...
        // ── Cache Busting：將 HTML 內所有 ?v=202607281155 替換成本次建置版本號 ──
    ...
    setTimeout(() => {
        fetch('cobblemon.js?v=' + Date.now())
            .then(r => r.text())
            .then(src => {
                // ── Cache Busting：將 JS 內所有 ?v=202607281155 替換成本次建置版本號 ──
...
window.onload = async function () {
    ...
    if (!data) {
        try {
            const res = await fetch('cobblemon_data.json?v=202607281155');
