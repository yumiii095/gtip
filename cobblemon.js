...
// 將檔案內所有 ?v=202604272201 統一替換為新版本號
// 比對範圍：?v= 後面非空白且非引號、結尾或 & 之前的字元
function _stampCacheBuster(text, ver) {
    return text.replace(/\?v=202604272201"'\s&)<>]+/g, '?v=' + ver);
}
...
function executeFinalSave() {
    ...
    setTimeout(() => {
        ...
        // ── Cache Busting：將 HTML 內所有 ?v=202604272201 替換成本次建置版本號 ──
    ...
    setTimeout(() => {
        fetch('cobblemon.js?v=' + Date.now())
            .then(r => r.text())
            .then(src => {
                // ── Cache Busting：將 JS 內所有 ?v=202604272201 替換成本次建置版本號 ──
...
window.onload = async function () {
    ...
    if (!data) {
        try {
            const res = await fetch('cobblemon_data.json?v=202604272201');
